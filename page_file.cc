	...
	void Page_File::load_baselines(){
		...
		for (pugi::xml_node text_region = page.child("TextRegion"); text_region; text_region = text_region.next_sibling("TextRegion"))
		{
			...
			//		this->baseline_order[this->baseline_order.size()-1].push_back(std::make_pair(max_coord_y,this->baseline_order[this->baseline_order.size()-1].size()));
			    	this->baseline_order[this->baseline_order.size()-1].push_back(std::make_pair(coord_mean_y,this->baseline_order[this->baseline_order.size()-1].size()));
...
*/
	...
	float Page_File::calculate_avg_interline_space(){
		vector<cv::Point> temp_contour;
		float avg_res = 0;
		float avg_count = 0;
		vector < vector < cv::Point> > tmp_points;
		...
		for(int j = 0 ; j < this->baseline_order.size(); j++){
			cv::Rect containing_rect =  boundingRect(regions[j]);
			//int k = this->region_order[j].second;
			LOG4CXX_INFO(this->logger,"Lines in region "<< j  << " :  " << this->baselines[j].size() << " : " << this->baseline_order[j].size());
			for(int i = 0 ; i < this->baseline_order[j].size(); i++){
				 int h = this->baseline_order[j][i].second;
			     LOG4CXX_INFO(this->logger,"In region "<< j  << " Line :  " << i  << " in possiton : " << h);
			}
			last = containing_rect.y;
			...
			for(int i = 0 ; i < this->baseline_order[j].size(); i++){
				 ...
				 LOG4CXX_INFO(this->logger,"Reading line " << i << " In order : " << h << "That has num points : " << sorted_baseline.size());
				 LOG4CXX_INFO(this->logger,"Reading line " << i << " In order : " << h << "That has num points : " << this->baselines[j][i].size());
				 ...
				 last = bottom;
				 avg_count+=1.0;
				 avg_res+=interline_space; 
			}

		}

		return avg_res/avg_count; 
	}

	void Page_File::generate_countour_from_baseline(int descendant_offset, int ascendant_offset ){
		vector<cv::Point> temp_contour;
		clean_contours();
		float avg_int_space = calculate_avg_interline_space();
		...
		for(int j = 0 ; j < this->baseline_order.size(); j++){
			...
			for(int i = 0 ; i < this->baseline_order[j].size(); i++){
				 ...
				 float interline_space = bottom - last;
				 interline_space = interline_space < 0.3 * avg_int_space ? avg_int_space : interline_space; 
				 float next_interline_space = next_bottom - bottom; 
				 next_interline_space = next_interline_space < 0.3 * avg_int_space ? avg_int_space : next_interline_space; 
				 ...
				 pugi::xml_attribute baseline_points_attr = this->line_nodes[j][h].child("Baseline").attribute("points");
				 baseline_points_attr.set_value(point_vectors_to_string(sorted_baseline).c_str());
//is for hs
				 pugi::xml_attribute line_points_attr = this->line_nodes[j][h].child("Coords").attribute("points");
				 vector<cv::Point> new_contour;
				 LOG4CXX_INFO(this->logger,"Generating contour for  " << this->line_nodes[j][h].attribute("id").value() << " points : " << sorted_baseline.size() );
				 for(int p=0; p < sorted_baseline.size(); p++){
			...
			LOG4CXX_INFO(this->logger,"Lines in region "<< j  << " :  DONE");
		}
	}
...
// minMaxLoc(InputArray src, double* minVal, double* maxVal=0, Point* minLoc=0, Point* maxLoc=0, InputArray mask=noArray())
	...
	void Page_File::calculate_reading_order(){
		LOG4CXX_INFO(this->logger," Sorting lines");
		calculate_line_order();
		LOG4CXX_INFO(this->logger," Sorting Regions");
